...
import { LANDING_CONFIG as CONFIG, SERVICOS_LANDING as SERVICOS, STATS, DEPOIMENTOS, GALERIA } from './data';
    ...
    <>
      <nav className={`nav ${scrolled ? 'nav-scrolled' : ''}`}>
        <div className="nav-inner">
          <a href="#" className="nav-logo" onClick={(e) => { e.preventDefault(); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>
            <img src="/logo.png" alt="Eraldo Refrigeração"/>
          </a>
          ...
          <a href="#admin" className="nav-cta" onClick={(e) => { e.preventDefault(); onAdmin(); }}>
      ...
      {menuOpen && (
        <div className="mobile-menu">
          ...
          <a href="#admin" onClick={(e) => { e.preventDefault(); onAdmin(); }} style={{ color: 'var(--brand-cyan)', fontWeight: 600 }}>
            Área Admin →
          </a>
        </div>
      )}
    ...
    <section className="section" id="diferenciais" style={{ paddingTop: 40 }}>
      <div className="section-inner">
        <div ref={ref} className="stats-band">
          {STATS.map((s, i) => <StatItem key={i} s={s} inView={inView}/>)}
        </div>
      </div>
    </section>
  );
            ...
            <div className="tst-card" key={i}>
              ...
              <div className="tst-stars">
                {Array.from({ length: d.stars }).map((_, j) => (
                  <Icon key={j} name="star" size={16} stroke={0} style={{ fill: '#F5B90A' }}/>
                ))}
              </div>
...
function Formulario() {
  const [form, setForm] = useState({ nome: '', fone: '', veiculo: '', servico: 'Recarga de Gás', msg: '' });
  const submit = (e) => {
    e.preventDefault();
      ...
      <div className="section-inner">
        <div className="map-card">
          <div className="map-info">
            ...
            <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(CONFIG.endereco + ', ' + CONFIG.cidade)}`} target="_blank" rel="noopener noreferrer" className="btn-primary">
  ...
  return (
    <footer className="footer">
      ...
      <div className="footer-bottom">
        <div>© {new Date().getFullYear()} {CONFIG.empresa}. CNPJ {CONFIG.cnpj}</div>
...
function FabWhats() {
  ...
  return (
    <a href={wpp} target="_blank" rel="noopener noreferrer" className="fab-whats">
